...
export const ProductEDit: React.FC<Props> = ({ className, productId }) => {
	const { product, onSubmit } = useAdminProductEdit(productId)
	...
	useEffect(() => {
		if (product) {
			...
			if (
				...
				currentValues.imageUrls !== product?.imageUrls ||
				currentValues.categoryId !== product?.category.id ||
				currentValues.items !== product?.items
			) {
				form.reset({
					name: product?.name || '',
					description: product?.description || '',
